...
export const CreatePollPage = ({ onNavigate }) => {
  ...
  // Success State after poll creation
  const [createdPoll, setCreatedPoll] = useState(null);
  const [copied, setCopied] = useState(false);

  const handleAddOption = () => {
    if (options.length >= 8) {
      showToast('Maximum 8 options allowed per poll', 'info');
      return;
    }
  ...
  const validate = () => {
    ...
    if (emptyOptions) {
      errs.options = 'All option choices must have text';
    }
  ...
  const shareableUrl = createdPoll ? `${origin}/#poll-${createdPoll.id}` : '';
  ...
  if (createdPoll) {
    return (
      <div className="create-poll-container animate-fade-in">
        <Card className="poll-success-card glass-panel" glow>
          <div className="success-badge-wrapper">
            <span className="success-icon-circle">🎉</span>
          </div>

          <h2 className="success-title">Poll Created Successfully</h2>
          <p className="success-subtitle">
            Your live poll is active and ready to accept responses from anywhere in the world.
          </p>

          <div className="success-poll-details">
            <span className="success-question-label">Question:</span>
            <h3 className="success-question-text">"{createdPoll.question}"</h3>
          </div>

          {/* Shareable URL with Copy Button */}
          <div className="success-url-box">
            <div className="success-url-wrapper">
              <GlobeIcon size={16} className="success-url-icon" />
              <input
                type="text"
                readOnly
                value={shareableUrl}
            ...
            <Button
              variant="secondary"
              icon={ShareIcon}
              onClick={() => openShareModal(createdPoll)}
            >
              Share
            </Button>
          </div>
          ...
          <div className="success-qr-box">
            <h4 className="success-qr-title">Instant QR Code</h4>
            <QRCodeCard value={shareableUrl} size={150} />
            <p className="success-qr-desc">Audience can scan with their phone camera to vote without signing up</p>
          </div>
    ...
    <div className="create-poll-container animate-fade-in">
      <div className="create-poll-header">
        ...
        <p className="create-poll-subtitle">
          Configure questions, interactive choices, and distribution settings in seconds.
        </p>
      </div>

      <form onSubmit={handleSubmit} className="create-poll-form" noValidate>
        {/* Main Card: Question & Options */}
        <Card className="create-poll-card glass-panel" glow>
          <div className="form-section-header">
            <h3 className="form-section-title">1. Poll Question</h3>
            <span className="section-step-pill">Step 1 of 2</span>
          </div>
          ...
          <div className="input-group">
            <label className="input-label">Optional Description / Context</label>
            <textarea
              rows={2}
          ...
          <div className="options-section">
            ...
            {errors.options && (
              <p className="input-error-msg mb-2">{errors.options}</p>
            )}

            <div className="options-input-list">
              {options.map((opt, idx) => (
                <div key={idx} className="option-row animate-fade-in">
        ...
        {/* Settings Card */}
        <Card className="create-poll-card glass-panel">
          <div className="form-section-header">
            <h3 className="form-section-title">2. Settings & Privacy</h3>
            <span className="section-step-pill">Step 2 of 2</span>
          </div>

          <div className="poll-settings-list">
            <div className="setting-select-group">
              <div className="setting-text">
                <label className="toggle-label">Poll Expiry</label>
                <p className="toggle-description">Automatically close the poll after a specified time frame</p>
              </div>
              <select
                value={expiry}
